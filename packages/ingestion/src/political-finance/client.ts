...
function generateIncomeBreakdown(
  ...
  const base = isCommunist
    ? [0.15, 0.05, 0.0, 0.02, 0.0, 0.01, 0.5, 0.27]
    : isSmall
      ? [0.1, 0.15, 0.05, 0.05, 0.3, 0.1, 0.15, 0.1]
      : [0.08, 0.1, 0.15, 0.1, 0.25, 0.12, 0.1, 0.1];
...
function generateExpenditureBreakdown(random: () => number): { category: string; ratio: number }[] {
  const base = [0.2, 0.1, 0.05, 0.08, 0.15, 0.25, 0.1, 0.07];
...
export function getReportsWithDetails(): {
  ...
  for (const org of orgs) {
    ...
    for (const year of FISCAL_YEARS) {
      ...
      const expenditureRatio = 0.8 + random() * 0.15;
